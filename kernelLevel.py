...
scraping_start_time=0
scraping_thread_name=''

def task_wrapper(func, args,intended_start_time, start_time, task_name, metrics):
    # print(task_name)
    thread_start_time = time.time()
    if(task_name)=='Scraping task':
        scraping_start_time=thread_start_time
    func(*args)
    duration = time.time() - thread_start_time
    with metrics['lock']:
        metrics['end_times'][threading.current_thread().name] = time.time()
        metrics['durations'][threading.current_thread().name] = duration
        metrics['waiting_times'][threading.current_thread().name] = thread_start_time - intended_start_time
...
def computation_task():
    ...
    time.sleep(random.random())  # Simulate computation time variability

def io_task(filename):
    with open(filename, "a") as file:
        file.write(f"Log entry at {time.ctime()}\n")
    time.sleep(random.random())  # Simulate I/O variability

def data_processing_task(data):
    data.sort()
    time.sleep(random.random())  # Simulate processing time variability

def network_task():
    requests.get("https://www.usenix.org/system/files/conference/atc18/atc18-bouron.pdf")
...
def image_processing_task(image_path):
    ...
    time.sleep(random.random())  # Simulate image processing time

def scrape(airline_codes, airportCodes,metrics,threads):

    # def computation(airline_codes,airportCodes):
    ...
    scraping_thread_name=current_thread.name
    ...
    # for i in range(0,len(airportCodes),5):
    #     thread = threading.Thread(target=computation, args=(airline_codes,airportCodes[i:i+5]))
    #     threads.append(thread)
    #     thread.start()

def embarrassingly_parallel_task(metrics,threads):
    def parallel_computation():
        count = 0
        for _ in range(1000000 // 4):  # Divide workload among threads
            x = random.random()
            y = random.random()
    ...
    for i in range(4):
        thread = threading.Thread(target=parallel_computation)
        threads.append(thread)
        thread.start()
...
def main():
    ...
    tasks = [
        ...
        (io_task, ("log1.txt",), 'IO Task'),
        (network_task, (), 'Network Task'),
        (data_processing_task, ([random.randint(1, 1000) for _ in range(100)],), 'Data Processing Task'),
        (image_processing_task, ("image.jpeg",), 'Image Processing Task'),
        (embarrassingly_parallel_task,(metrics,threads,),'embarrassingly_parallel_task'),
        (scrape,(airline_codes,airportCodes[0:5],metrics,threads,), 'Scraping task1'),
        (scrape,(airline_codes,airportCodes[5:10],metrics,threads,), 'Scraping task2'),
        (scrape,(airline_codes,airportCodes[10:15],metrics,threads,), 'Scraping task3'),
        (scrape,(airline_codes,airportCodes[15:20],metrics,threads,), 'Scraping task4'),
        (scrape,(airline_codes,airportCodes[20:],metrics,threads,), 'Scraping task5')
    ]

    for func, args, name in tasks:
        intended_start_time=time.time()
        thread = threading.Thread(target=task_wrapper, args=(func, args, intended_start_time, start_time, name, metrics))
        threads.append(thread)
        thread.start()
    ...
    duration = time.time() - scraping_start_time
    metrics['end_times'][scraping_thread_name] = time.time()
    metrics['durations'][scraping_thread_name] = duration
    print(scraping_thread_name)
    print(f"Scraping task completed in {duration:.4f} seconds.")

    total_time = time.time() - start_time
    print(f"Total execution time: {total_time:.4f} seconds")
    throughput = len(tasks) / total_time
    print(f"Throughput: {throughput:.4f} tasks/second")

    # Calculate turnaround and waiting times
    for thread_name, end_time in metrics['end_times'].items():
        turnaround_time = end_time - start_time
        print(f"Turnaround time for {thread_name}: {turnaround_time:.4f} seconds")

    # CPU Utilization
    cpu_usage = psutil.Process(os.getpid()).cpu_percent(interval=1) / psutil.cpu_count()
    print(f"CPU Utilization: {cpu_usage:.2f}%")

    total_waiting_time = sum(metrics['waiting_times'].values())
    print(f"Total waiting time across all threads: {total_waiitng_time:.4f} seconds")
